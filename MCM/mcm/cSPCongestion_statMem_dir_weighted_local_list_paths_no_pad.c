            ...
            for(p=0; p<reqDataPack; p++){     
                ...
                if(mxGetN(currAccessibleDest) > 0 && mxGetM(currAccessibleDest) > 0){
                    ...
                    if(randIndex != -1){
                        ...
                        mxArray * sp_cell = mxGetCell(PathList_st, 0);

                        mwSize num_rows_path = mxGetM(sp_cell);
                        mwSize num_cols_path = mxGetN(sp_cell);    

                        float *sp_data = mxGetSingles(sp_cell);

                        // Get dimensions of the matrix (num_paths x L)
                       

                        // Filter candidate rows
                        double threshold = sp_data[0 + (num_cols_path - 1)*num_rows_path]*(1+tol_value);  // <-- chosen distance cutoff
                        mwIndex imax = 0;
                        
                        for (mwIndex i = 0; i < num_rows; i++) {
                            mxArray * p_cell = mxGetCell(PathList_st, i*1 + 0);
                            float *data = mxGetSingles(p_cell);
                            num_rows_path = mxGetM(p_cell);
                            num_cols_path = mxGetN(p_cell);  

                            double path_length = data[0 + (num_cols_path - 1) * num_rows_path]; // last column entry
                            
                            if (path_length > threshold) {// chosing between the paths within the tolerance distance
                                imax = i;
                                if (imax == 0 && num_rows > 1){
                                    sprintf(outputString,"fprintf('Warning: just 1 path within tolerance for packet from %i to %i\\n')",
                        ...
                        // random row [0, imax-1]
                        mwIndex chosen_path_i = 0;

                        if (imax > 0){
                            chosen_path_i = rand() % imax;
                        }

                        mxArray * p_cell = mxGetCell(PathList_st, chosen_path_i*num_cols + 0*num_rows);// get the chosen path
                        float *data = mxGetSingles(p_cell);
                        ...
                        if (n > randDest) {// if the source > dest follow the path backwards
                            mwIndex index_in_path = -1;
                            for (mwIndex j = 0; j < num_cols_path - 1; j++) { // -1 to skip the length column
                                if (data[0 + j * num_rows_path] == n) {
                                    index_in_path = j;
                                    break;
                                }
                            }
                            if (index_in_path == -1) {
                                ...
                                mexPrintf("number of columns: %lu\n", num_cols_path);
                                mexPrintf("Path %d→%d (row %lu):\n", newPacket.source, newPacket.destination, chosen_path_i);
                                for (mwIndex j = 0; j < num_cols_path - 1; ++j) {
                                    mexPrintf("  col %lu: %d\n", j, (int)data[0 + j * num_rows_path]);
                                }

                                sprintf(outputString, "fprintf('  length = %.3f\\n')",
                                        data[0 + (num_cols_path - 1) * num_rows_path]);
                                mexEvalString(outputString);
                                
                                mexErrMsgIdAndTxt("cSP:pathUnderflow",
                                                 "Routing %d→%d could not find source node in chosen path. Source = %d",
                                                 newPacket.source, newPacket.destination, n);
                        ...
                        newPacket.pathRow = chosen_path_i; // <-- this is the row index in the PathList_st in cell (s,t)       
                ...
                if(!empty(&networkQueues[n])){
                    ...
                    if(newPacket_ptr->deliveredOnTime < t){
                        ...
                        if(newPacket_ptr->whereAmI == newPacket_ptr->destination){
                            ...
                            // sprintf(outputString,"fprintf('Packet from %i to %i reach destination\\n')",
                            //        newPacket_ptr->source,newPacket_ptr->destination);
                            // mexEvalString(outputString); 
                            ...
                            //}
                            // get list of paths from s to t. in cell (s,t) of PathLisArr (if s>t, cell (t,s))
                            ...
                            mwSize num_rows = mxGetM(PathList_st);
                            mwSize num_cols = mxGetN(PathList_st);
                            mxArray * p_cell = mxGetCell(PathList_st, newPacket_ptr->pathRow*num_cols + 0*num_rows);// get the chosen path
                            float *data = mxGetSingles(p_cell);

                            mwSize num_rows_path = mxGetM(p_cell);
                            mwSize num_cols_path = mxGetN(p_cell); 
        
                            //get next destination
                            if (newPacket_ptr->source > newPacket_ptr->destination){// if s>t follow the path backwards
                                nextDest = data[0 + (newPacket_ptr->indWhereAmI-1) * num_rows_path]; 
                            }else{
                                nextDest = data[0 + (newPacket_ptr->indWhereAmI+1) * num_rows_path];
                            }  
                            ...
                            if(nextDest >= 0 && nextDest < sizeSupra){                                
                              ...
                              if(1){   
                               ...
                               sigma[t*sizeSupra + nextDest] ++;                                                                    
                                ...
                                if(DEBUG){
                                    sprintf(outputString,"fprintf('Packet from = %i, to = %i, on %i, next dest = %i\\n')",
                                        newPacket_ptr->source,newPacket_ptr->destination,newPacket_ptr->whereAmI,nextDest);
                                    mexEvalString(outputString);                                                           
                                }
                                ...
                                if (newPacket_ptr->source > newPacket_ptr->destination) {
                                    if (newPacket_ptr->indWhereAmI == 0) {
                                        mexErrMsgIdAndTxt("cSP:pathUnderflow",
                                                        "Routing %d→%d fell off the beginning of its path",
                                                        newPacket_ptr->source, newPacket_ptr->destination);
                                    }
                                    newPacket_ptr->indWhereAmI--;
                                    // nextDest = data[0 + newPacket_ptr->indWhereAmI * num_rows_path];
                                } else {
                                    if (newPacket_ptr->indWhereAmI >= num_cols_path - 2) {
                                        mexErrMsgIdAndTxt("cSP:pathOverflow",
                                                        "Routing %d→%d exceeded stored path length",
                                                        newPacket_ptr->source+1, newPacket_ptr->destination+1);
                                    }
                                    newPacket_ptr->indWhereAmI++;
                                    // nextDest = data[0 + newPacket_ptr->indWhereAmI * num_rows_path];
                                }
                            ...
                            //   }
                                ...
                                sprintf(outputString,"fprintf('nextDest = %i\\n')",nextDest);
                                mexEvalString(outputString);   
