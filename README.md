...
Networks provide a natural framework for representing complex systems, and centrality measures quantify the importance of nodes and links across a wide range of fields. Geodesic-based metrics, such as shortest-path betweenness (SP-BW), assume that elements moving through a network always follow shortest routes. However, this assumption does not always hold. In transportation networks, empirical evidence shows that real mobility often deviates from optimal paths by about 15–20\%, yet this is not considered in current network metrics. In this work, we study Quasi-Shortest Path Betweenness (QSP-BW), a centrality measure that accounts for all paths within a given tolerance of the shortest path, and apply it to the analysis of transportation networks. We show that the number of quasi-shortest paths grows exponentially with tolerance across a broad class of network models, derive an analytical expression for random graphs, and explain why efficient algorithms cannot be directly implemented. By comparing QSP-BW with SP-BW, we demonstrate that increasing tolerance raises overall node betweenness and network load, leading to reduced network performance. However, in networks with prioritized routes, we identified a two-phase behaviour in which performance either improves or deteriorates depending on transport demand. Finally, we show that limiting path overlap can extend this improvement regime to homogeneous networks, highlighting both the potential and the inherent limitations of routing diversification as a congestion-mitigation strategy. Overall, our results demonstrate that moving beyond shortest-path routing fundamentally changes centrality-based predictions, strongly suggesting a reassessment of standard network centrality measures.
